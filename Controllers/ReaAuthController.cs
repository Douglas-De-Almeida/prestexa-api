...
namespace PrestexaAPI.Controllers
{
    ...
    public class ReaAuthController : ControllerBase
    {
        ...
        private readonly ILogger<ReaAuthController> _logger;

        private const int OtpTokenMinutes = 10;
        private const int PasswordResetTokenMinutes = 30;
        private const string ReaPortal = "rea";
        private static readonly TimeSpan ReaRememberDuration = TimeSpan.FromDays(7);

        public ReaAuthController(
            AppDbContext context,
            IConfiguration config,
            IEmailSender emailSender,
            IAuthTokenService authTokenService,
            ITrustedDeviceService trustedDeviceService,
            ILogger<ReaAuthController> logger)
        {
            _context = context;
            _config = config;
        ...
        public async Task<IActionResult> Login([FromBody] PortalLoginRequest request)
        {
            ...
            if (isTrustedDevice)
            {
                ...
                return Ok(new
                {
                    ...
                    mfaRemembered = true,
                    message = "REA login successful. Verification was remembered for this device."
                });
            }
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] PortalForgotPasswordRequest request)
        {
            ...
            try
            {
                var resetToken = CreatePasswordResetToken(user, "rea_password_reset");

                var resetBaseUrl =
                    _config["REA_PASSWORD_RESET_URL"] ??
                    _config["Auth:ReaPasswordResetUrl"] ??
                    "https://auth.prestexa.com/crm/reset-password";
                ...
                await _emailSender.SendEmailAsync(
                    user.Email,
                    "Reset your Prestexa CRM password",
                    $"We received a request to reset your CRM password. Use this link within {PasswordResetTokenMinutes} minutes: {resetUrl}",
                    $@"
                        <div style=""font-family:Arial,sans-serif;color:#111827;line-height:1.5;"">
                            <h2>Reset your CRM password</h2>
                            <p>We received a request to reset your real estate agent password.</p>
            ...
            {
                _logger.LogError(ex, "Failed to send REA password reset email for {Email}", normalizedEmail);
